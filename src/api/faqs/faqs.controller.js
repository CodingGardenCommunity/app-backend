...
async function getFAQs(req, res, next) {
  try {
    ...
    if ('id' in req.params) {
      ...
      try {
        response = [await FAQs.findById(req.params.id).exec()];
        if (response[0] === null) throw new ReferenceError('The requested ID does not exist.');
      } catch ({ message }) {
        throw new ReferenceError(message);
      }
    } else response = await FAQs.find({}).exec();
...
async function addFAQ(req, res, next) {
  try {
    ...
    const { _id } = await new FAQs({ question, answer }).save();
    res.json({
      status: 200,
...
async function removeFAQ(req, res, next) {
  try {
    ...
    const deletedFAQ = await FAQs.deleteOne({ _id });
