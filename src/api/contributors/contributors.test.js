...
describe('GET /contributors', () => {
  it('Should respond with a 200 status code', async () => {
    const { status, type, body: { data } } = await request(app).get('/contributors');
...
describe('GET /contributors/:id', () => {
  it('Should respond with a 200 status code for valid id', async () => {
    const { status, type, body: { data } } = await request(app).get('/contributors/w3cj');
    expect(status).toEqual(200);
    expect(type).toEqual('application/json');
    expect(data[0].id).toEqual('w3cj');
  });
